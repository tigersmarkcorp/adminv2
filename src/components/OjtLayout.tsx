...
import { OjtSidebar } from "@/components/OjtSidebar";
import { Loader2, Flame } from "lucide-react";

export default function OjtLayout() {
  const { user, isOjt, loading } = useAuth();
      ...
      <div className="min-h-screen flex w-full bg-background">
        ...
        <SidebarInset className="flex-1">
          <header className="sticky top-0 z-30 h-14 flex items-center border-b border-border/60 px-4 md:px-6 bg-card/80 backdrop-blur-xl">
            ...
            <h2 className="text-sm font-semibold text-foreground">OJT Portal</h2>
          </header>
