...
import { Search, CalendarClock, Eye, ArrowUpDown } from "lucide-react";
...
export default function FlexibleSchedule() {
  ...
  const fetchEmployees = async () => {
    ...
    if (data) setEmployees(data.filter((e: any) => e.schedule_type === "Flexible"));
  };

  useEffect(() => {
    fetchEmployees();
    const channel = supabase.channel("flexible-realtime").on("postgres_changes", { event: "*", schema: "public", table: "employees" }, fetchEmployees).subscribe();
    ...
    <div className="space-y-5">
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
        <h2 className="text-xl md:text-2xl font-bold text-foreground flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-warning flex items-center justify-center"><CalendarClock className="w-4 h-4 text-white" /></div>
          Flexible Schedule
        </h2>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {filtered.length === 0 && (
                <TableRow><TableCell colSpan={6} className="text-center py-12 text-muted-foreground">No flexible schedule employees found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
